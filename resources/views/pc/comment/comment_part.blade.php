...
<div class="comment-part-bor">
    @foreach($comments as $comment)
        @include('pc.comment.comment_part_item')
    @endforeach
</div>
{{--根据登录与否和是否为本回答的回答者给出提示添加评论--}}
@if(Auth::check())
    @if(\App\Helpers\Helpers::get_answer($commentable_id)->user_id != Auth::user()->id)
        <span><a href="javascript:void(0);" id="new-comment" class="new-comment"><i
                        class="iconfont icon-iconfontpinglun"></i>您有话要说？请添加您的评论吧......</a></span>
...
<div class="form-group comment-input">
    <form id="comment-content" method="post" action="{{ url('comment/answer_store') }}">
        ...
        <div class="comment-part-bottom">
            ...
            <button type="button" class="btn btn-reply" data-entity_id="{{ $commentable_id }}"
                    data-entity_type="{{ $commentable_type }}">回复
            </button>
        </div>
    </form>
</div>
