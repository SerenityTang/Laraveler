        ...
        <div class="row">
            ...
            <div class="col-md-9">
                <div class="panel panel-default right-container">
                    <h4 class="title">@if($user->id != (Auth::check() ? Auth::user()->id : 0)) TA的关注 @else
                            我的关注 @endif</h4>
                                ...
                                @foreach($atte_users as $atte_user)
                                    <li class="list-group-item list-tab">
                                        <div class="row">
                                            <div class="col-md-1">
                                                <a class="att-u-name"
                                                   href="{{ url('user/'.\App\Helpers\Helpers::get_user($atte_user->entityable_id)->personal_domain) }}">
                                                    <img src="{{ App\Helpers\Helpers::get_user_avatar($atte_user->entityable_id, 'medium') }}"
                                                         class="avatar-46"
                                                         alt="{{ \App\Helpers\Helpers::get_user($atte_user->entityable_id)->username }}">
                                                </a>
                                            </div>
                                            <div class="col-md-8">
                                                <a class="att-u-name"
                                                   href="{{ url('user/'.\App\Helpers\Helpers::get_user($atte_user->entityable_id)->personal_domain) }}">{{ \App\Helpers\Helpers::get_user($atte_user->entityable_id)->username }}</a>
                                                <p class="data">
                                            ...
                                            <div class="col-md-1 col-md-offset-2">
                                                <a class="attention-icon" style="margin-bottom: 3px;"
                                                   href="javascript:void(0)" data-user="{{ $atte_user->entityable_id }}"
                                ...
                                @foreach($atte_ques as $atte_que)
                                    <li class="list-group-item list-tab">
                                        <div class="row content">
                                            <div class="col-md-7">
                                                <a href="{{ url('question/show/' . $atte_que->entityable_id) }}"
                                                   title="{{ \App\Helpers\Helpers::get_question($atte_que->entityable_id)->title }}"
                                                   class="title">{{ str_limit(\App\Helpers\Helpers::get_question($atte_que->entityable_id)->title, 60) }}</a>
                                            </div>
                                            ...
                                            <div class="col-md-2 create-time"
                                                 title="{{ \App\Helpers\Helpers::get_question($atte_que->entityable_id)->created_at }}">
