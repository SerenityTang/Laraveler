                ...
                <div class="panel panel-default">
                    <div class="panel-heading search-heading">
                        <div id="tabs" class="tabs">
                            <nav>
                                <ul>
                                    <li @if($filter === 'all') class="tab-current" @endif><a href="{{ route('search.show') }}?q={{ $keyword }}"><span>全部</span></a></li>
                                    <li @if($filter === 'question') class="tab-current" @endif><a href="{{ route('search.show', ['filter' => 'question']) }}?q={{ $keyword }}"><span>问答</span></a></li>
                                    <li @if($filter === 'blog') class="tab-current" @endif><a href="{{ route('search.show', ['filter' => 'blog']) }}?q={{ $keyword }}"><span>博客</span></a></li>
                                    <li @if($filter === 'tag') class="tab-current" @endif><a href="{{ route('search.show', ['filter' => 'tag']) }}?q={{ $keyword }}"><span>标签</span></a></li>
                                </ul>
                            </nav>
                        </div>
                    </div>
                        ...
                        <ul class="list-group">
                            @if(isset($ques_searchs))
                                @foreach($ques_searchs as $question)
                                    <li class="list-group-item global-list-item">
                                        <h2 class="title">
                                            <a href="{{ url('question/show/' . $question->id) }}" title="{{ $question->title }}">{!! str_replace($keyword, '<span class="keyword">'.$keyword.'</span>', $question->title) !!}</a>
                                        </h2>
                            ...
                            @if(isset($blog_searchs))
                                @foreach($blog_searchs as $blog)
                                    <li class="list-group-item global-list-item">
                                        <h2 class="title">
                                            <a href="{{ url('blog/show/' . $blog->id) }}" title="{{ $blog->title }}">{!! str_replace($keyword, '<span class="keyword">'.$keyword.'</span>', $question->title) !!}</a>
                                        </h2>
