...
@if( Auth::check() && \App\Helpers\Helpers::get_answer($entity_id)->user_id != Auth::user()->id )
...
@elseif(Auth::check() && \App\Helpers\Helpers::get_answer($entity_id)->comment_count < 1)
...
<script>
    ...
    $(function () {
        ...
        $('.media-comment .comment-part-bor span a.reply-icon').click(function () {
            ...
            @else
                if ($('.media-comment .comment-input').is(":hidden")){
                    ...
                    $('.media-comment .comment-input').toggle();
                } else {
                    $('.media-comment textarea').attr('placeholder', '@ '+user_name);
                }
            @endif
        ...
        $('#new-comment').click(function () {
            ...
            @else
                if ($('.media-comment .comment-input').is(":hidden")){
                    $('.media-comment textarea').attr('placeholder', '请输入您的评论......');
                    $('.media-comment .comment-input').toggle();
                } else {
                    $('.media-comment textarea').attr('placeholder', '请输入您的评论......');
                }
