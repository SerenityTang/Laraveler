@if(isset($best_answer))
    <ul class="list-group best-answer">
        <li class="list-group-item best-answer-bg">
            ...
            <div class="media">
                <a class="media-left ans-avatar avatar-40" href="{{ url('user/'.$best_answer->user->personal_domain) }}">
                ...
                <div class="media-body">
                    <h4 class="media-heading best-answer-author">
                        <a class="author-name" href="{{ url('user/'.$best_answer->user->personal_domain) }}">{{ $best_answer->user->username }}</a>
                    </h4>
                    {!! $best_answer->content !!}
                    <h4 class="media-heading author">
                        <a class="author-name" href="{{ url('user/'.$question->user->personal_domain) }}">{{ $question->user->username }}</a>
